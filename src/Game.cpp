...
void Game::spawnEnemy()
{
    ...
    while (costLeft > 0)
    {
        randomCost = rand() % (std::min(costLeft, 4)) + 1;
        std::unique_ptr<Enemy> enemy;
        Position positionEnemy, posEnemyPlayer;
        do
        {
        ...
        enemy->adjustPositionBasedOnWalls(walls);
        enemy->adjustPositionBasedOnOOB();
